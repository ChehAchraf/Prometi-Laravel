...
class TimeEntryController extends Controller
{
    public function __construct()
    {
        ...
        // Only admin, HR, and pointage editors can manage time entries
        $this->middleware('role:superadmin,hr_editor,pointage_editor')->except(['index', 'show']);
        
        // Everyone can view their own time entries
        $this->middleware('role:superadmin,hr_editor,pointage_editor,project_viewer,technical_director')->only(['index', 'show']);
    }
    ...
    public function index(Request $request)
    {
        ...
        if (in_array($userRole, [Role::SUPERADMIN, Role::HR_EDITOR, Role::TECHNICAL_DIRECTOR])) {
    ...
    public function create()
    {
        ...
        if (in_array($userRole, [Role::SUPERADMIN, Role::HR_EDITOR])) {
            ...
            // Chef de chantier can only add time entries for their projects
            $projects = Project::whereHas('users', function($query) use ($user) {
                $query->where('users.id', $user->id);
            })->get();
    ...
    public function edit(TimeEntry $timeEntry)
    {
        ...
        if (!in_array($userRole, [Role::SUPERADMIN, Role::HR_EDITOR])) {
            // Chef de chantier can edit entries for their projects
            if ($userRole === Role::POINTAGE_EDITOR) {
                $hasAccess = Project::where('id', $timeEntry->project_id)
                    ->whereHas('users', function($query) use ($user) {
                        $query->where('users.id', $user->id);

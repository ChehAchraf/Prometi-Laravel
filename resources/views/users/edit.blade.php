        ...
        <form action="{{ route('users.update', $user) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="role" id="role" required
                    ...
                    <option value="worker" {{ old('role', $user->role) == 'worker' ? 'selected' : '' }}>Ouvrier</option>
                </select>
                @error('role')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <textarea name="address" id="address" rows="3"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">{{ old('address', $user->address) }}</textarea>
                @error('address')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>

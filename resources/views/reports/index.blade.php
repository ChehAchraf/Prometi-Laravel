...
@section('content')
    <div class="p-6">
        ...
        <!-- Report Filters -->
        <form action="{{ route('reports.export') }}" method="GET" class="bg-white rounded-lg shadow-md p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                    <label for="start_date" class="block text-sm font-medium text-gray-700 mb-1">Période du</label>
                    <div class="relative">
                        <input type="date" id="start_date" name="start_date"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 py-2 px-3 border"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="text-sm text-gray-500">
                        ...
                        <span
                            class="ml-2 {{ $stats['overtimeHours']['change'] >= 0 ? 'text-green-500' : 'text-red-500' }}">
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="text-sm text-gray-500">
                        ...
                        <span
                            class="ml-2 {{ $stats['activeEmployees']['change'] >= 0 ? 'text-green-500' : 'text-red-500' }}">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($topPerformers as $employee)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <img class="h-10 w-10 rounded-full" src="{{ $employee->avatar_url }}"
                                                    alt="">
                                            </div>
                                            <div class="ml-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ number_format($employee->total_hours, 0) }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">
                                            {{ number_format($employee->overtime_hours, 0) }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <span
                                                class="text-sm font-medium text-gray-900">{{ $employee->performance }}%</span>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                                        ...
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-8 w-8">
                                                <img class="h-8 w-8 rounded-full"
                                                    src="{{ $project->manager_avatar_url }}" alt="">
                                            </div>
                                            <div class="ml-3">
                                                <div class="text-sm font-medium text-gray-900">{{ $project->chef->name }}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
...
@push('scripts')
    ...
    <script>
        ...
        const projectHoursChart = new Chart(ctx, {
            type: 'bar',
            data: {
            ...
            tabs.forEach(tab => {
                document.getElementById(`tab-${tab}`).addEventListener('click', function() {
                    ...
                    tabs.forEach(t => {
                        ...
                        document.getElementById(`tab-${t}`).classList.add(
                    ...
                    document.getElementById(`tab-${tab}`).classList.remove('border-transparent',
                        'text-gray-500');
                    document.getElementById(`tab-${tab}`).classList.add('border-primary-500',
                        'text-primary-600');
                });
            });

...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <p class="text-gray-600">Update time entry information</p>
    </div>
        ...
        <form action="{{ route('time-entries.update', $timeEntry) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="status" id="status" required
                    ...
                    <option value="present" {{ old('status', $timeEntry->status) == 'present' ? 'selected' : '' }}>Present</option>
                    <option value="absent" {{ old('status', $timeEntry->status) == 'absent' ? 'selected' : '' }}>Absent</option>
                    <option value="late" {{ old('status', $timeEntry->status) == 'late' ? 'selected' : '' }}>Late</option>
                    <option value="early_leave" {{ old('status', $timeEntry->status) == 'early_leave' ? 'selected' : '' }}>Early Leave</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
